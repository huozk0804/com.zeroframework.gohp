...
namespace ZeroFramework.Goap
{
        ...
        where U : unmanaged, IComparer<T>
    {
...
#endif
                var indexToRemove = Data->Table[index.TableIndex].HeapIndex;

                var toRemove = Data->Heap[indexToRemove];

#if ENABLE_UNITY_COLLECTIONS_CHECKS
                Data->Table[toRemove.TableIndex].Version++;
#endif

                var lastNode = Data->Heap[--Data->Count];
        ...
        internal unsafe void IsValidIndexInternal(NativeHeapIndex index, ref bool result, ref int errorCode)
        {
            ...
            var tableValue = Data->Table[index.TableIndex];
