...
namespace ZeroFramework.Editor.Package
{
    public class ActionList : ListElementBase<CapabilityAction, CapabilityActionElement>
    {
        ...
        {
            return new CapabilityActionElement(serializedObject, property, scriptable, generator, item);
        }
        ...
        protected override void BindListItem(SerializedProperty property, CapabilityActionElement element, CapabilityAction item, int index)
        {
            ...
            element.Action.Bind(scriptable, item.action, generator.GetActions(), classRef =>
            {
                element.Foldout.text = item.action.Name;
                element.Properties.Bind(property, item, generator.GetActions());
            });
            
            element.Properties.Bind(property, item, generator.GetActions());
            
            element.Target.Bind(scriptable, item.target, generator.GetTargetKeys(), classRef =>
            {
                element.Foldout.text = item.action.Name;
            });
