...
namespace ZeroFramework.Editor.Package
{
    public class WorldDataDrawer : VisualElement
    {
        public WorldDataDrawer(ILocalWorldData worldData)
        {
            ...
            var card = new Card((card) =>
            {
                ...
                card.schedule.Execute(() =>
                {
                    ...
                    root.Add(CreateTable(new string[] { "Key", "Value", "Age" }, GetValues(worldData)));
                }).Every(500);
        ...
        {
            return GetWorldValues(worldData).Concat(GetTargetValues(worldData)).ToArray();
        }

        private string[][] GetWorldValues(ILocalWorldData worldData)
        {
            return worldData.States.Select(pair => GetValues(pair.Key, pair.Value)).ToArray();
        }

        private string[][] GetTargetValues(ILocalWorldData worldData)
        {
            return worldData.Targets.Select(pair => GetValues(pair.Key, pair.Value)).ToArray();
        }

        private string[] GetValues<T>(System.Type key, IWorldDataState<T> dataState)
        {
            var local = dataState.IsLocal ? "Local" : "Global";

            return new string[]
            {
                $"{Goap.Extensions.GetGenericTypeName(key)} ({local})",
                GetValueText(dataState),
                GetText(dataState.Timer),
            };
        }
