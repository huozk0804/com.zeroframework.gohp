...
namespace ZeroFramework.Goap
{
    public abstract class MultiSensorBase : IMultiSensor
    {
        ...
        public Type[] GetKeys()
        {
            ...
            foreach (var sensor in LocalSensors.Keys)
            {
                keys.Add(sensor);
            }

            foreach (var sensor in GlobalSensors.Keys)
            {
                keys.Add(sensor);
            }
        ...
        {
            foreach (var sensor in GlobalSensors.Values)
            {
                sensor.Sense(data);
            }
        }
        ...
        public void Sense(IWorldData data, Type[] keys)
        {
            ...
            {
                if (GlobalSensors.ContainsKey(key))
                {
                    GlobalSensors[key].Sense(data);
                }
            }
        }
        ...
        {
            foreach (var sensor in LocalSensors.Values)
            {
        ...
        public void Sense(IWorldData data, IActionReceiver agent, IComponentReference references, Type[] keys)
        {
            foreach (var key in keys)
            {
                if (LocalSensors.ContainsKey(key))
                {
                    LocalSensors[key].Sense(data, agent, references);
                }
            }
        }
        ...
        {
            ValidateCalledFromConstructor();

            timer ??= SensorTimer.Always;

            LocalSensors.Add(typeof(TKey), new LocalSensor
            {
                Key = typeof(TKey),
            ...
            where TKey : IWorldKey
        {
            ...
            GlobalSensors.Add(typeof(TKey), new GlobalSensor
            {
                Key = typeof(TKey),
